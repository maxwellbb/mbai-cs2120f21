...
/-
We know that w = z. We also know that the symmetric property of
equality is true. Therefore, since w = z, we know that z = w.
-/
...
/-
Let's take an arbitrary object in some domain, and if it can be
proven that object has some property P, then we can assume all
other objects in that domain have property P.
-/
...
but specific propositions, and that we have proofs,
p and qr, of P and Q ∧ R. By application of the
∧ eliminiation rule, we can find a proof, q and r,
for Q and R, respectively. Then, using the ∧ intro
rule, we can prove (P ∧ Q). Then, to obtain a proof
of R, we can use the ∧ elim rule on qr to obtain
r, a proof of R. QED. 
-/
